...
function main(request, response) {
  ...
  response.write('<link rel="shortcut icon" type="image/x-icon" href="' + strApacheRootLink + 'favicon.ico"/>');
	...
	///  Tab Bar with CSS : Start  ////////////////////////////////////
  	header_css( response, "button_help");
  	/////////// Tab Br with CSS : End  ////////////////////////////////////
...
*/
	...
	response.write('		<td width="420" height="218" colspan="4" bgcolor="#1B1B1B"><font color="#c6c6c6"><em>Welcome to the iBox Support Machine!</em><br><br>Please use the links above to get access to the User manual, Quick Start guide, YouTube help videos, our Support Forum, FAQ, and Wiki resources. External content requires an internet connection. iBox Support is handled via support forums where you can get help from iBox Team members and fellow users.<br><br>Sincerely,<br><br>The iBox Team</font></td>');
	response.write('		<td rowspan="4">');
	...
	*/
	...
	eXcOut = execSync(strCMD);
		console.log(eXcOut);
		response.write('<FONT STYLE="' + strFontStyle + '" SIZE="-1" COLOR="#666666"><center>Nano Uptime: ' + eXcOut + '</center></font>')
		//response.write('<FONT STYLE="' + strFontStyle + '" SIZE="-1" COLOR="#666666">Nano Uptime: ' + eXcOut + '</font>')
	}catch (err) {
	...
	eXcOut = execSync(strCMD);
		...
		eXcOut = eXcOut.replace(/(\r\n|\n|\r)/g,"<br>"); // or text = text.replace(/\r\n?|\n/g, '<br />');
		//  Now convert tabs to spaces
		eXcOut = eXcOut.replace(/\t/g, '&nbsp;&nbsp;&nbsp;&nbsp;');
		response.write('<FONT STYLE="' + strFontStyle + '" SIZE="-1" COLOR="#666666"><center>Nano Memory Usage:<br>' + eXcOut + '</center></font>')
	}catch (err) {
		console.log('ERROR [' + err + '] : executing execSync[' + strCMD + ']')
	}
